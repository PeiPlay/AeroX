...
*/
void _FdcanDrv_InitStandardFrame(FDCAN_HandleTypeDef *hfdcan)
{
    ...
    {		
		...
		.FilterType = (hfdcan->Instance != FDCAN3) ? FDCAN_FILTER_MASK : FDCAN_FILTER_RANGE,
		.FilterID1 = 0x00,
        .FilterID2 = (hfdcan->Instance != FDCAN3) ? 0x00 : 0x7FF
    })) Error_Handler();
}
/*
...
*   FDCAN1的使用FIFO0缓冲区，FDCAN2和FDCAN3关联到FIFO1缓冲区
*/
void _FdcanDrv_InitExtendedFrame(FDCAN_HandleTypeDef *hfdcan)
{    
    if(HAL_OK != HAL_FDCAN_ConfigFilter(hfdcan, &(FDCAN_FilterTypeDef)
    {
        ...
        .FilterType = (hfdcan->Instance != FDCAN3) ? FDCAN_FILTER_MASK : FDCAN_FILTER_RANGE,
        .FilterID1 = 0x00,
        .FilterID2 = (hfdcan->Instance != FDCAN3) ? 0x00 : 0x1FFFFFFF
    })) Error_Handler();
}
/*
*   对传入的FDCAN外设进行默认初始化：
...
void FdcanDrv_Init_Default(FDCAN_HandleTypeDef *hfdcan)
{
    ...
    _FdcanDrv_InitStandardFrame(hfdcan);
    _FdcanDrv_InitExtendedFrame(hfdcan);
...
{
    return FdcanDrv_Transmit(hfdcan, FDCAN_TRANSMIT_ID_STD | FDCAN_TRANSMIT_FRAME_DATA | FDCAN_TRANSMIT_CAN_CLASSIC, id, pData, dlcField);
}
...
{
    return FdcanDrv_Transmit(hfdcan, FDCAN_TRANSMIT_ID_STD | FDCAN_TRANSMIT_FRAME_REMOTE | FDCAN_TRANSMIT_CAN_CLASSIC, id, pData, dlcField);
}
HAL_StatusTypeDef FdcanDrv_Transmit_Classic_ExtData(FDCAN_HandleTypeDef *hfdcan, uint32_t id, void *pData, uint32_t dlcField)
{
    return FdcanDrv_Transmit(hfdcan, FDCAN_TRANSMIT_ID_EXT | FDCAN_TRANSMIT_FRAME_DATA | FDCAN_TRANSMIT_CAN_CLASSIC, id, pData, dlcField);
}
...
{
    return FdcanDrv_Transmit(hfdcan, FDCAN_TRANSMIT_ID_EXT | FDCAN_TRANSMIT_FRAME_REMOTE | FDCAN_TRANSMIT_CAN_CLASSIC, id, pData, dlcField);
}
HAL_StatusTypeDef FdcanDrv_Transmit_FD_StdData(FDCAN_HandleTypeDef *hfdcan, uint32_t id, void *pData, uint32_t dlcField)
{
    return FdcanDrv_Transmit(hfdcan, FDCAN_TRANSMIT_ID_STD | FDCAN_TRANSMIT_FRAME_DATA | FDCAN_TRANSMIT_CAN_FD, id, pData, dlcField);
}
HAL_StatusTypeDef FdcanDrv_Transmit_FD_StdRemote(FDCAN_HandleTypeDef *hfdcan, uint32_t id, void *pData, uint32_t dlcField)
{
    return FdcanDrv_Transmit(hfdcan, FDCAN_TRANSMIT_ID_STD | FDCAN_TRANSMIT_FRAME_REMOTE | FDCAN_TRANSMIT_CAN_FD, id, pData, dlcField);
}
HAL_StatusTypeDef FdcanDrv_Transmit_FD_ExtData(FDCAN_HandleTypeDef *hfdcan, uint32_t id, void *pData, uint32_t dlcField)
{
    return FdcanDrv_Transmit(hfdcan, FDCAN_TRANSMIT_ID_EXT | FDCAN_TRANSMIT_FRAME_DATA | FDCAN_TRANSMIT_CAN_FD, id, pData, dlcField);
}
HAL_StatusTypeDef FdcanDrv_Transmit_FD_ExtRemote(FDCAN_HandleTypeDef *hfdcan, uint32_t id, void *pData, uint32_t dlcField)
{
    return FdcanDrv_Transmit(hfdcan, FDCAN_TRANSMIT_ID_EXT | FDCAN_TRANSMIT_FRAME_REMOTE | FDCAN_TRANSMIT_CAN_FD, id, pData, dlcField);
}
